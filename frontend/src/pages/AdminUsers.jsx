...
export default function AdminUsers() {
  ...
  const fetchFromApi = useCallback(async () => {
    ...
    let res = await fetch(`${API}/admin/users`, opts);
    if (res.status === 404) res = await fetch(`${API}/users`, opts);
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      const msg = data.error || (res.status === 401 ? "Session expired. Please log in again." : res.status === 403 ? "Access denied. Admin only." : "Failed to load users");
      throw new Error(msg);
    }
    ...
    return (
      <div className={styles.page}>
        ...
        {error.includes("Session expired") && (
          <p className={styles.hint}>Go to Login and sign in again with admin credentials.</p>
        )}
      </div>
    );
  }
