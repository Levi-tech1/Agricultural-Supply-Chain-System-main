/**
 * Vercel serverless proxy: forwards same-origin /api/* to your Express backend.
 * Keep a single catch-all under /api — do not add another file like api/[...all].mjs
 * (Vercel fails the build: conflicting paths / names).
 * Set BACKEND_URL in Vercel → Project → Settings → Environment Variables
 * to the API origin only (no trailing slash, no /api suffix), e.g. https://your-api.onrender.com
 * If BACKEND_URL mistakenly includes /api, it is stripped so paths are not doubled.
 */
